...
# This global variable will hold data needed by the worker function
_frame_generation_data = None

def _generate_single_frame(odom_idx):
    """
    Worker function to generate a single frame.
    Uses global _frame_generation_data to access shared data.
    """
    import matplotlib.pyplot as plt
    from matplotlib.patches import Circle
    
    # Unpack shared data
    odom_x, odom_y = _frame_generation_data['odom']
    local_goals_x, local_goals_y = _frame_generation_data['goals']
    valid_obstacles = _frame_generation_data['obstacles']
    lidar_readings = _frame_generation_data['lidar']
    output_folder = _frame_generation_data['output_folder']
    x_min, x_max = _frame_generation_data['x_limits']
    y_min, y_max = _frame_generation_data['y_limits']
    obstacle_radius = _frame_generation_data['obstacle_radius']
    draw_ray = _frame_generation_data['draw_ray']
...
def generate_frames_obst_optimized(odom_x, odom_y, local_goals_x, local_goals_y, obstacles, 
    ...
    # Create a figure to determine axis limits
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111)
    ax.plot(odom_x, odom_y, 'b-')
    ax.plot(local_goals_x, local_goals_y, 'r-')
    
    # Add obstacles to get correct axis limits
    for obstacle in valid_obstacles:
        circle = Circle((obstacle.centerPoint[0], obstacle.centerPoint[1]), 
                       obstacle_radius, fill=False, color='r')
        ax.add_patch(circle)
    ...
    # Set up the shared data for worker processes
    _frame_generation_data = {
        'odom': (odom_x, odom_y),
        'goals': (local_goals_x, local_goals_y),
    ...
    # Generate frames with specified step size
    frame_indices = list(range(0, len(odom_x), frame_step))
    
    print(f"Generating {len(frame_indices)} frames...")
    
    # Sequential version as fallback
    if multiprocessing.get_start_method() == 'spawn' or len(frame_indices) < 10:
        # Sequential processing is more reliable on some platforms
        for idx in frame_indices:
            _generate_single_frame(idx)
    else:
        try:
            # Try parallel processing with a smaller number of processes
            num_cpus = max(1, min(4, multiprocessing.cpu_count() - 1))
            print(f"Using {num_cpus} processes for parallel frame generation")
            
            with ProcessPoolExecutor(max_workers=num_cpus) as executor:
                results = list(executor.map(_generate_single_frame, frame_indices))
    ...
    print(f"Generated frames in {output_folder}/")
...
def main():
    ...
    generate_frames_obst_optimized(odom_x, odom_y, local_goals_x, local_goals_y, obstacleArray, lidar_readings, frame_dkr) 
